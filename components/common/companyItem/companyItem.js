...
Component({
  ...
  methods: {
    ...
    touchendFn: function (e) {
      let id = this.properties.userItem.companyId;
      if (this.data.isClick) {
        this.initData()
        wx.navigateTo({
          url: '/pages/company/companyDetail/companyDetail?contactId=' + id + '&contactType=1',
        })
        return;
      }
      let left = this.data.nowLeft;
    ...
    editUser: function (e) {
      this.initData()
      wx.navigateTo({
        url: '/pages/company/companyAdd/companyAdd?id=' + this.properties.userItem.companyId,
      })
